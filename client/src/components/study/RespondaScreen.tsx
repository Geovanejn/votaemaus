...
export function RespondaScreen({
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8F9FC] dark:bg-zinc-950">
      ...
      <div 
        className="relative px-6 pt-10 pb-12 rounded-b-[32px] overflow-hidden shadow-sm"
        ...
        <div className="max-w-md mx-auto flex flex-col items-center">
          <div className="w-full flex justify-between items-center mb-6">
            ...
            <div className="text-center">
              <p className="text-white/80 text-[10px] font-bold uppercase tracking-wider mb-1">Questão</p>
              <p className="text-white text-2xl font-black">{currentIndex + 1} / {totalQuestions}</p>
            </div>
          ...
          {showHearts && (
            <div className="flex gap-1.5 mb-6">
              {Array.from({ length: maxHearts }).map((_, i) => (
                <Heart key={i} className={cn("h-6 w-6", i < hearts ? "text-[#FF4B4B] fill-[#FF4B4B]" : "text-white/30")} />
              ))}
            </div>
          )}
      ...
      <div className="max-w-md mx-auto w-full px-5 -mt-6 flex-1 flex flex-col">
        <Card className="border-0 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[24px] bg-white dark:bg-zinc-900 p-6 mb-6">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-pink-400 flex items-center justify-center text-white font-bold text-sm">?</div>
            <span className="text-[#7c3aed] text-[10px] font-black uppercase tracking-widest">{currentQuestion.category || "HISTÓRIA"}</span>
          </div>
          <h3 className="text-[19px] font-bold text-[#2D3142] dark:text-zinc-100 leading-[1.4]">
          ...
          {currentQuestion.options?.map((option, idx) => {
            ...
            return (
              <button
                ...
                disabled={showResult}
                className={cn(
                  "w-full p-4 rounded-[18px] text-left transition-all border-2 flex items-center gap-4 min-h-[72px]",
                  !showResult && isSelected ? "border-[#7c3aed] bg-white" : "border-white bg-white shadow-[0_4px_20px_rgb(0,0,0,0.03)]",
                  showCorrect && "border-[#22C55E] bg-[#F0FDF4]",
                  showIncorrect && "border-[#EF4444] bg-[#FEF2F2]"
                )}
              >
                <div className={cn(
                  "w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center font-black text-sm border-2",
                ...
                <span className="text-[#4B5563] font-bold text-[15px] leading-tight">{option}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="max-w-md mx-auto pointer-events-auto">
          {!showResult ? (
            <Button
              ...
              className="w-full h-[60px] rounded-[20px] bg-gradient-to-r from-[#7c3aed] to-[#f472b6] text-white text-lg font-bold shadow-lg shadow-purple-500/20 border-0"
            >
              Confirmar Resposta
            </Button>
          ) : (
            <Button
              onClick={handleNext}
              className="w-full h-[60px] rounded-[20px] bg-[#7c3aed] text-white text-lg font-bold shadow-lg shadow-purple-500/20 border-0"
            >
