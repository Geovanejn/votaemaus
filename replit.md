...
- **Fixed critical election finalization bug**: Added database migration in `server/db.ts` to create the missing `closed_at` column in the `elections` table, which was causing "no such column: closed_at" error when trying to finalize elections. The migration checks if the column exists before adding it to prevent duplicate column errors.
- **Fixed PDF title duplication**: Modified PDF generator (`client/src/lib/pdfGenerator.ts`) to prevent "Eleição: Eleição 2025/2026" duplication. Now checks if the election name already starts with "Eleição" and only adds the prefix if needed, preserving custom election names.
- **Improved export results image layout**: Adjusted candidate photo positioning in `ExportResultsImage.tsx` by lowering the circular photo 8px (from -50px/-40px to -58px/-48px) and reduced title line spacing by adding `lineHeight: "1"` to create tighter, more professional layout.
- **Filtered completed positions from candidate dialog**: Added smart filtering in admin panel's "Add Candidate" dialog to hide positions that already have elected winners (status: 'completed'), preventing duplicate candidate assignments. Includes fallback to show all positions while election data is loading.

## Previous Changes (November 3, 2025)
- **Improved PDF audit report with UMP Emaús branding**: Updated the election audit PDF generator to use the official UMP Emaús logo (copied to `client/public/logo-ump.png`) instead of the generic logo. The PDF now renders the logo with correct aspect ratio (no distortion). Changed subtitle from "União da Mocidade Presbiteriana Emaús" to "União de Mocidade Presbiteriana Emaús" (correct name). Reorganized signature page layout so the president's signature appears BEFORE the date/location information "São Paulo, SP, [data]". Improved spacing calculations to prevent layout issues with long names. Modified `loadImageAsBase64` function to return image dimensions for proper aspect ratio calculation.
