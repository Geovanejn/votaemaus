...
- **Optimized logo for email compatibility and visual consistency**: Replaced the 160KB logo in `client/public/logo.png` with an optimized 32KB version (full logo with "emaús" text). This reduces base64 email embedding from ~220KB to 42KB, significantly improving Gmail compatibility and load times. Added size logging to `server/email.ts` showing original and base64 sizes on startup. Added consistent logo footer to the results page matching the admin and vote pages (h-48 height). All pages now use the same optimized logo asset for consistent branding: login (top), admin/vote/results (footer), and email (footer).
