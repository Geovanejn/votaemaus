...
Each lesson follows a structured 3-stage format:

1. **ESTUDE (Study)**: Reading content and Bible verses
   - Text units with educational content
   - Verse units with ARA (Almeida Revista e Atualizada) Bible citations
   - Users gain XP but DO NOT lose hearts

2. **MEDITE (Meditate)**: Practical applications and prayer
   - Reflection units with life applications generated by AI
   - Meditation units with prayer guides
   - Users gain XP but DO NOT lose hearts

3. **RESPONDA (Answer)**: Questions and exercises
   - Multiple choice, true/false, and fill-in-the-blank exercises
   - ONLY stage where users can LOSE hearts on wrong answers
   - Users gain XP on correct answers

### Life Recovery System (3 Verses = +1 Heart)
- Users can recover hearts by reading Bible verses
- **3 verses must be read to recover 1 heart**
- Progress is tracked across verse readings
- Verses use the ARA (Almeida Revista e Atualizada) translation
- The `/study/verses` page shows recovery progress and available verses
...
- **StudyHeader**: Progress bar header with hearts display and stage indicators (Estude, Medite, Responda)
...
**Hearts Recovery (3 Verses = +1 Heart):**
- `GET /api/study/verses` - Get all Bible verses (ARA translation) for heart recovery
- `POST /api/study/verses/:verseId/read` - Read verse and progress toward heart recovery
- `GET /api/study/verses/recovery-progress` - Get current verse reading progress (versesRead, versesNeeded)
