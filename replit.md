...
- **Daily Verse System**: Automated daily Bible verse publishing with AI-generated reflections (5-prompt rotation: Pastor Acolhedor, Conselheiro Espiritual, Narrativo-Devocional, Motivacional-Inspirador, Teológico-Acessível — rotated by day-of-year mod 5; references limited to 1 per estrofe, author names rare), contextual AI-selected background images (Gemini analyzes verse context → generates Pexels search query → fetches relevant image → uploads to R2, with stock image fallback), deduplication via `imageSourceUrl` tracking (last 60 posts), public page, social media sharing, and push notifications.
