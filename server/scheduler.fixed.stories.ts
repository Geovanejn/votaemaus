...
// Normalize story images for Instagram Graph API (reduce size and ensure supported JPEG)
async function normalizeStoryImage(input: Buffer): Promise<Buffer> {
  // Instagram is strict with content-type/size. JPEG 1080x1920 is a safe baseline.
...
async function generateAndSaveStoryImages() {
  ...
  try {
    ...
    // CORREÇÃO AQUI: Forçando 'image/jpeg' e extensão .png
    const verseR2Url = await uploadStoryImageToR2(await normalizeStoryImage(verseBuffer), 'stories', 'image/jpeg', `verse-story-${todayKey}.jpg`);
    ...
    // CORREÇÃO AQUI: Forçando 'image/jpeg' e extensão .png
    const reflectionR2Url = await uploadStoryImageToR2(reflectionBuffer, 'stories', 'image/jpeg', `reflection-story-${todayKey}.jpg`);
  ...
  try {
    ...
    for (const member of members) {
      try {
        ...
        // CORREÇÃO AQUI: Forçando 'image/jpeg' e extensão .png para manter a transparência
        const filename = `birthday-${member.id}-${todayFullDate}.jpg`;
        const r2Url = await uploadStoryImageToR2(await normalizeStoryImage(buffer), 'stories', 'image/jpeg', filename);
