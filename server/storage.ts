...
export interface IStorage {
  ...
  // Daily Story Images (Instagram Stories cache)
  saveDailyStoryImage(data: { type: string; dateKey: string; imageUrl: string; r2Key?: string | null }): Promise<schema.DailyStoryImage>;
  getDailyStoryImage(type: string, dateKey: string): Promise<schema.DailyStoryImage | null>;

  // Birthday helpers for Instagram Stories
  getMembersByBirthday(day: number, month: number): Promise<Array<{ id: number; fullName: string; email: string | null }>>;
...
// Bible Verses (Recovery Pool - per user, per day)
private getSaoPauloNow(): Date {
  // Create a Date object representing "now" in America/Sao_Paulo (used for dateKey math)
  return new Date(new Date().toLocaleString('en-US', { timeZone: 'America/Sao_Paulo' }));
}

private getBrazilDateKey(date: Date = new Date()): string {
...
private async ensureRecoveryVersePoolForUser(
  userId: number,
  targetUnread: number = 50,
  dateKey?: string
): Promise<void> {
  const nowSP = this.getSaoPauloNow();
  const dayKey = dateKey ?? this.getBrazilDateKey(nowSP);

  // How many unread verses does the user have today?
  const [countRow] = await db
    .select({ count: sql<number>`count(*)` })
    .from(schema.userRecoveryVersePool)
    .where(and(
      eq(schema.userRecoveryVersePool.userId, userId),
      eq(schema.userRecoveryVersePool.dateKey, dayKey),
      eq(schema.userRecoveryVersePool.isRead, false)
    ));

  const unreadCount = Number(countRow?.count ?? 0);
  if (unreadCount >= targetUnread) return;

  const missing = targetUnread - unreadCount;

  // Avoid repeating verses recently shown to this user (best-effort).
  // If the catalog is small, we will gracefully fall back to older repeats.
  const avoidDays = 30;
  const minDate = new Date(nowSP);
  minDate.setDate(minDate.getDate() - avoidDays);
  const minDateKey = this.getBrazilDateKey(minDate);

  const todayPool = await db
    .select({ verseId: schema.userRecoveryVersePool.verseId })
    .from(schema.userRecoveryVersePool)
    .where(and(
      eq(schema.userRecoveryVersePool.userId, userId),
      eq(schema.userRecoveryVersePool.dateKey, dayKey)
  ...
  const todayVerseIds = todayPool.map(r => r.verseId);
  ...
  const recentPool = await db
    ...
    .where(and(
      eq(schema.userRecoveryVersePool.userId, userId),
      gte(schema.userRecoveryVersePool.dateKey, minDateKey)
    ));

  const recentVerseIds = recentPool.map(r => r.verseId);

  const buildNotIn = (col: any, ids: number[]) => {
    if (!ids.length) return null;
    return sql`${col} NOT IN (${sql.join(ids.map((id) => sql`${id}`), sql`, `)})`;
  };

  // 1) Try to pick verses not shown in the last X days and not already in today's pool
  const firstPassConds: any[] = [];
  const notInToday = buildNotIn(schema.bibleVerses.id, todayVerseIds);
  if (notInToday) firstPassConds.push(notInToday);

  const excludeRecent = Array.from(new Set(recentVerseIds));
  const notInRecent = buildNotIn(schema.bibleVerses.id, excludeRecent);
  if (notInRecent) firstPassConds.push(notInRecent);

  let candidates: { id: number }[] = [];

  if (firstPassConds.length > 0) {
    candidates = await db
      .select({ id: schema.bibleVerses.id })
      .from(schema.bibleVerses)
      .where(and(...firstPassConds))
      .orderBy(sql`RANDOM()`)
      .limit(Math.max(missing * 3, missing));
  } else {
    candidates = await db
      .select({ id: schema.bibleVerses.id })
      .from(schema.bibleVerses)
  ...
  // 2) If the catalog is too small, fall back to allowing older repeats (still avoid duplicating within today)
  if (candidateIds.length < missing) {
    const secondPassConds: any[] = [];
    if (notInToday) secondPassConds.push(notInToday);

    const secondCandidates = secondPassConds.length
      ? await db
          .select({ id: schema.bibleVerses.id })
          .from(schema.bibleVerses)
          .where(and(...secondPassConds))
          .orderBy(sql`RANDOM()`)
          .limit(Math.max(missing * 3, missing))
      : await db
          .select({ id: schema.bibleVerses.id })
    ...
    candidateIds = secondCandidates.map(c => c.id);
  }

  const idsToInsert = candidateIds.slice(0, missing);
  if (!idsToInsert.length) return;

  await db
    .insert(schema.userRecoveryVersePool)
    .values(idsToInsert.map((verseId) => ({
      userId,
      dateKey: dayKey,
      verseId,
      isRead: false,
...
async getUnreadVersesForUser(userId: number): Promise<any[]> {
  const dayKey = this.getBrazilDateKey(this.getSaoPauloNow());

  // Ensure the user always has 50 unread verses available today
  await this.ensureRecoveryVersePoolForUser(userId, 50, dayKey);

  // Return today's unread pool (up to 50)
  return await db
    .select({
      id: schema.bibleVerses.id,
      reference: schema.bibleVerses.reference,
      text: schema.bibleVerses.text,
      reflection: schema.bibleVerses.reflection,
      category: schema.bibleVerses.category,
    })
    .from(schema.userRecoveryVersePool)
    .innerJoin(schema.bibleVerses, eq(schema.userRecoveryVersePool.verseId, schema.bibleVerses.id))
    .where(and(
      eq(schema.userRecoveryVersePool.userId, userId),
      eq(schema.userRecoveryVersePool.dateKey, dayKey),
      eq(schema.userRecoveryVersePool.isRead, false)
...
async readVerseAndRecoverHeart(userId: number, verseId: number): Promise<any> {
  ...
  // Ensure pool exists for today (and has stock)
  await this.ensureRecoveryVersePoolForUser(userId, 50, dayKey);

  // Check if this verse is in today's pool and whether it was already read today
  const [poolEntry] = await db.select()
    .from(schema.userRecoveryVersePool)
    .where(and(
      eq(schema.userRecoveryVersePool.userId, userId),
      eq(schema.userRecoveryVersePool.dateKey, dayKey),
      eq(schema.userRecoveryVersePool.verseId, verseId)
    ))
    .limit(1);
  ...
  // Mark as read today (or create the entry if somehow missing)
  if (poolEntry) {
    await db.update(schema.userRecoveryVersePool)
      .set({ isRead: true, readAt: new Date() })
      .where(eq(schema.userRecoveryVersePool.id, poolEntry.id));
  } else {
    await db.insert(schema.userRecoveryVersePool)
      .values({
        userId,
        dateKey: dayKey,
        verseId,
  ...
  // Record reading history (for achievements / audit)
  await db.insert(schema.verseReadings)
    .values({ userId, verseId, readAt: new Date() });

  const profile = await this.getStudyProfile(userId);
  if (!profile) {
    // Still replenish the pool to keep 50 available
    await this.ensureRecoveryVersePoolForUser(userId, 50, dayKey);
  ...
  async saveBirthdayShareImage(
    memberIdOrData: number | { memberId: number; dateKey: string; imageUrl: string; r2Key?: string },
    imageUrl?: string,
    shareDate?: string
  ): Promise<schema.BirthdayShareImage> {
    const isObject = typeof memberIdOrData === "object";
    const memberId = isObject ? memberIdOrData.memberId : memberIdOrData;
    const dateKey = isObject ? memberIdOrData.dateKey : (shareDate as string);
    const url = isObject ? memberIdOrData.imageUrl : (imageUrl as string);
    ...
    const existing = await db.select()
      ...
      .where(and(
        ...
        eq(schema.birthdayShareImages.shareDate, dateKey)
      ))
      .limit(1);
    ...
    const [created] = await db.insert(schema.birthdayShareImages)
      .values({ memberId, imageUrl: url, shareDate: dateKey })
      .returning();
    return created;
  }
  ...
  async saveDailyStoryImage(data: { type: string; dateKey: string; imageUrl: string; r2Key?: string | null }): Promise<schema.DailyStoryImage> {
    const [existing] = await db.select()
      .from(schema.dailyStoryImages)
      .where(and(
        eq(schema.dailyStoryImages.type, data.type),
        eq(schema.dailyStoryImages.dateKey, data.dateKey)
      ))
      .limit(1);

    if (existing) {
      const [updated] = await db.update(schema.dailyStoryImages)
        .set({
          imageUrl: data.imageUrl,
          r2Key: data.r2Key ?? existing.r2Key ?? null,
        })
        .where(eq(schema.dailyStoryImages.id, existing.id))
        .returning();
      return updated;
    }

    const [created] = await db.insert(schema.dailyStoryImages)
      .values({
        type: data.type,
  ...
  async getDailyStoryImage(type: string, dateKey: string): Promise<schema.DailyStoryImage | null> {
    const [img] = await db.select()
      .from(schema.dailyStoryImages)
      .where(and(
        eq(schema.dailyStoryImages.type, type),
        eq(schema.dailyStoryImages.dateKey, dateKey)
      ))
      .limit(1);
    return img || null;
  }
  ...
  async getMembersByBirthday(day: number, month: number): Promise<Array<{ id: number; fullName: string; email: string | null }>> {
    const dd = String(day).padStart(2, "0");
    const mm = String(month).padStart(2, "0");

    // Supported formats in users.birthdate: YYYY-MM-DD or DD/MM/YYYY (as used elsewhere in routes)
    const isoSuffix = `-${mm}-${dd}`;
    const brPrefix = `${dd}/${mm}`;
    ...
    const rows = await db.select({
      ...
      isMember: schema.users.isMember,
    })
      .from(schema.users)
      .where(and(
        eq(schema.users.isMember, true),
        eq(schema.users.activeMember, true),
        sql`${schema.users.birthdate} IS NOT NULL`,
        or(
          sql`${schema.users.birthdate} LIKE ${'%' + isoSuffix}`,
          sql`${schema.users.birthdate} LIKE ${brPrefix + '%'}`
        )
      ));

    return rows.map(r => ({ id: r.id, fullName: r.fullName, email: r.email ?? null }));
  }
