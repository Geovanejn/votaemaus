...
import { pgTable, serial, integer, text, boolean, unique, timestamp, real, jsonb, index, uniqueIndex } from "drizzle-orm/pg-core";
...
// Stores generated story images (verse/reflection/birthday) for a given day, so scheduled publishing can reuse them.
export const dailyStoryImages = pgTable("daily_story_images", {
  id: serial("id").primaryKey(),
  type: text("type").notNull(), // 'verse' | 'reflection' | 'birthday'
  dateKey: text("date_key").notNull(), // YYYY-MM-DD (America/Sao_Paulo)
  imageUrl: text("image_url").notNull(), // public URL
  r2Key: text("r2_key"), // internal R2 key/path
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (table) => ({
  typeDateUnique: uniqueIndex("daily_story_images_type_date_unique").on(table.type, table.dateKey),
  typeDateIdx: index("daily_story_images_type_date_idx").on(table.type, table.dateKey),
}));

export const insertDailyStoryImageSchema = createInsertSchema(dailyStoryImages).omit({
  id: true,
  createdAt: true,
});

export type DailyStoryImage = typeof dailyStoryImages.$inferSelect;
export type InsertDailyStoryImage = z.infer<typeof insertDailyStoryImageSchema>;
