...
export const userRecoveryVersePool = pgTable("user_recovery_verse_pool", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  dateKey: text("date_key").notNull(), // YYYY-MM-DD (America/Sao_Paulo)
  verseId: integer("verse_id").notNull().references(() => bibleVerses.id, { onDelete: "cascade" }),
  isRead: boolean("is_read").notNull().default(false),
  readAt: timestamp("read_at"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (table) => ({
  uniqueUserDayVerse: unique().on(table.userId, table.dateKey, table.verseId),
  lookupIdx: index("user_recovery_verse_pool_lookup_idx").on(table.userId, table.dateKey, table.isRead),
  userDayIdx: index("user_recovery_verse_pool_user_day_idx").on(table.userId, table.dateKey),
}));

export const insertUserRecoveryVersePoolSchema = createInsertSchema(userRecoveryVersePool).omit({
  id: true,
  createdAt: true,
});

export type InsertUserRecoveryVersePool = z.infer<typeof insertUserRecoveryVersePoolSchema>;
export type UserRecoveryVersePool = typeof userRecoveryVersePool.$inferSelect;
